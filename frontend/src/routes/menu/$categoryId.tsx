...
function RouteComponent() {
  ...
  return (
    <div className='h-full w-full grid grid-cols-2 auto-rows-min overflow-auto justify-items-center items-start mt-[4%] pb-24 gap-4 px-3 bg-[#F4EAE0]'>
      {items.map((item) => (
        <div key={item.id} className='w-36 h-28 bg-[#E8D8C4] flex flex-col justify-between items-center rounded-2xl text-[#7A5C3E] shadow-md border border-[#C4956A]/30 p-3'>
          <div className='flex flex-col justify-center items-center gap-1 w-full text-center'>
            <span className='font-bold text-xs text-[#2C2118] tracking-wide leading-tight'>{item.name}</span>
            <span className='text-sm font-bold text-[#C4956A]'>{item.price}</span>
          </div>
          <div className='flex flex-row gap-3 justify-center items-center'>
            <button onClick={() => handleDecrease(item.id)} className='h-8 w-8 bg-[#E8828A] rounded-full text-white text-lg font-bold shadow-sm active:scale-95 transition-transform'>-</button>
            <span className='text-sm font-bold text-[#4A3728] w-4 text-center'>{itemsQuantity[item.id] || 0}</span>
            <button onClick={() => handleIncrease(item.id)} className='h-8 w-8 bg-[#3A7D32] rounded-full text-white text-lg font-bold shadow-sm active:scale-95 transition-transform'>+</button>
          </div>
        </div>
      ))}
